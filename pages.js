...
/* ============================================================
   Blocked view — one vertical "well" per category, scrolled
   horizontally. Blocks are squares whose side is the task's weight in
   grid units (weight 5 spans the full 6-unit width), packed densely so
   lighter tasks tetris into the gaps beside heavier ones.
   ============================================================ */

// Soonest deliver-by first (so overdue leads), heaviest first on ties,
// completed sink to the bottom.
function blockedOrder(cat) {
  return catBoxTasks(cat).slice().sort((a, b) =>
    (a.done ? 1 : 0) - (b.done ? 1 : 0)
    || dueDateOf(a).localeCompare(dueDateOf(b))
    || b.weight - a.weight);
}

function renderBlockedView(host) {
  const rail = el('div', 'block-rail');
  state.categories.forEach((cat) => rail.appendChild(blockedWell(cat)));
  host.appendChild(rail);
}

function blockedWell(cat) {
  const col = el('div', 'block-col');
  col.dataset.cat = cat.id;
  const tasks = blockedOrder(cat);

  const head = el('div', `block-head tint-${cat.color}`);
  head.innerHTML = `<span class="block-dot"></span><span class="block-name">${escapeHtml(cat.name)}</span>`
    + `<span class="block-count">${tasks.filter((t) => !t.done).length} / ${tasks.length}</span>`;
  const add = el('button', 'block-add');
  add.textContent = '＋';
  add.title = `Add a ${cat.name} task`;
  add.onclick = (e) => { e.stopPropagation(); openModal({ categoryId: cat.id }); };
  head.appendChild(add);
  col.appendChild(head);

  const well = el('div', 'block-well');
  if (!tasks.length) {
    const empty = el('div', 'block-empty');
    empty.textContent = 'nothing here';
    well.appendChild(empty);
  } else {
    const grid = el('div', 'block-grid');
    tasks.forEach((t) => grid.appendChild(taskBlock(t, cat)));
...
function taskBlock(t, cat) {
  const tk = todayKey();
  const due = dueDateOf(t);
  const overdue = due < tk && !t.done;
  // Routine carries no weight, so those tasks all render at the base 1x1 size.
  const size = catHasWeight(cat.id) ? Math.max(1, Math.min(5, t.weight)) : 1;

  const b = el('div', `task-block s${size} tint-${overdue ? 'red' : cat.color}`
    + (t.done ? ' done' : '') + (overdue ? ' overdue' : ''));
  b.title = `${t.title}\n${prettyDate(due)} · ${relativeDue(due)}`
    + (catHasWeight(cat.id) ? ` · weight ${t.weight}` : '')
    + (t.note ? `\n\n${t.note}` : '');
  b.onclick = () => toggleTask(t.id);

  const top = el('div', 'tb-top');
  const title = el('div', 'tb-title');
  title.textContent = t.title;
  top.appendChild(title);
  // only the biggest blocks have room for the weight note
  if (size >= 4 && t.note && state.settings.showWeightNotes) {
    const note = el('div', 'tb-note');
    note.textContent = t.note;
  ...
  const dueEl = el('span', 'tb-due');
  dueEl.textContent = size === 1 ? relativeDueShort(due) : relativeDue(due);
  foot.appendChild(dueEl);
  if (catHasWeight(cat.id)) {
    const w = el('span', 'tb-w');
    w.textContent = t.weight;
    foot.appendChild(w);
  }
  b.appendChild(foot);

  if (size >= 2) {
    const edit = el('button', 'tb-edit');
    edit.textContent = '✎';
    edit.title = 'Edit';
